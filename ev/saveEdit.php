<?php
	...
	sec_session_start();
	if(login_check($db) === false) { //we are not logged in, so throw an error
		header('Location: error.php?err=Update proposal failed: LOGIN ERROR');
	}
	
	$evtStmt = $db->prepare("SELECT webTitle, coordinatorEmail FROM events WHERE event = ?");
	$evtStmt->bind_param('s', $_POST["prop_type"]);
	$evtStmt->execute();
	...
	for($i = 1; $i < count($tmp); $i++) { //the first element is blank
		...
		if($thisP['id'] == "undefined") { //this presenter doesn't exist, so we need to insert
			...
			if(!$pQ_stmt->execute()) {
				header('Location: error.php?err=Presenter registration failure: INSERT'.$db->error);
			}
			
			$presStr .= $db->insert_id."|";
		} else { // presenter already exists, so update
			$pQ_stmt = $db->prepare("UPDATE presenters SET `Prefix` = ?, `First Name` = ?, `Last Name` = ?, `Title` = ?, `City` = ?, `State` = ?, `Province` = ?, `Postal Code` = ?, `Country` = ?, `Phone` = ?, `Extension` = ?, `Fax` = ?, `Email` = ?, `Member` = ?, `Student` = ?, `Affiliation Name` = ?, `Affiliation Country` = ?, `Publish Email` = ?, `First Time` = ? WHERE `id` = ?");
	...
	$q_stmt = $db->prepare("UPDATE `proposals` SET `title` = ?, `contact` = ?, `presenters` = ?, `times` = ?, `topics` = ?, `computer` = ?, `summary` = ?, `abstract` = ?, `comments` = ? WHERE `id` = ?");
	
	$timesStr = trim(strip_tags($_POST["prop_times"]),"|");
	$topicsStr = trim(strip_tags($_POST["prop_topics"]),"|");
	
	$q_stmt->bind_param('ssssssssss',strip_tags($_POST['prop_title']), $mainContact, trim($presStr,"|"), $timesStr, $topicsStr, $_POST["prop_computer"], strip_tags($_POST["prop_summary"]), strip_tags($_POST["prop_abstract"]), strip_tags($_POST["prop_comments"]),$_POST["prop_id"]);
	...
	if(strip_tags($_POST["prop_password"]) != "") { //a new password was entered
		...
		//We couldn't include it above because we only update it if a new password was entered
		
		$q_stmt = $db->prepare("UPDATE `proposals` SET `password` = ?, `salt` = ? WHERE `id` = ?");
		$q_stmt->bind_param('sss',$password, $random_salt, $_POST["prop_id"]);
		if(!$q_stmt->execute()) {
			echo $q_stmt->error;
			exit();
		}
	...
	*/
	...
	$subject = "Electronic Village ".$confYear.": ".$_POST["prop_type"]." (Submission Edited)";
	...
	*/
	...
	$tmpMsg = file_get_contents("saveEditEmail.txt");
	$tmpPres = "";
	$tmpPresRows = "";
	...
	if($tmpTopics != "") $tmpTopics = "Presentation Topics:\n".$tmpTopics;
	...
	if($mail_sent) {
		//We are done with any session variables, and we want to log the user out before we send the confirmation page
		logout(); //now that the information has been saved, log the user out
?>
<html>
	<head>
		<title>Electronic Village Proposals -- <?php echo $_POST["prop_type"]; ?></title>
		<style type="text/css">
			body {
	...
	<body>
		<div id="pagecontainer">
			<table border="0" align="center" cellspacing="0" cellpadding="0" width="800">
				...
				<tr>
					<td align="center" style="padding-top: 20px; padding-bottom: 20px"><span style="font-size: 24pt; font-weight: bold">CALL-IS Electronic Village Events (<?php echo $confYear; ?>)<br /><br /><span style="font-size: 18pt; font-weight: bold"><?php echo $_POST["prop_type"]; ?> (Submission Confirmation)</span></td>
				</tr>
				<tr>
					<td style="padding: 20px">Your changes have been saved! Below is the information you submitted for your reference. An email has also been sent to <span style="font-weight: bold; color: blue"><?php echo $mainContact; ?></span> with the same information. Please save a copy of this information for your records. If you need to edit your submission again (<span style="font-weight: bold; color: red">only available up until the submission deadline</span>), please go to <a href="http://call-is.org/ev/edit.php?id=<?php echo $propID; ?>">http://call-is.org/ev/edit.php?id=<?php echo $propID; ?></a> and enter the main contact email and your password.<br /><br />If you have questions about your submission, or general questions about the <?php echo $_POST["prop_type"]; ?> event, please contact <a href="mailto:<?php echo $from; ?>"><?php echo $from; ?></a>.</td>
				</tr>
				<tr>
					...
					<td id="topicsTD" style="border-top: solid 1px #CCCCCC; padding: 20px">
						...
						<table border="0" cellspacing="0" cellpadding="5" width="100%">
							<tr>
								<td>
<?php
		$tmpTopics = explode("|",strip_tags(stripslashes($_POST["prop_topics"]))); //strip tags because of the "other" option
...
?>
				<tr>
					<td id="summaryTD" style="border-top: solid 1px #CCCCCC; padding: 20px">
						...
						<table border="0" cellspacing="0" cellpadding="5" width="100%">
							<tr>
								<td><?php echo strip_tags(stripslashes($_POST["prop_summary"])); ?></td>
							</tr>
						</table>
					</td>
				</tr>
				<tr>
					<td id="abstractTD" style="border-top: solid 1px #CCCCCC; padding: 20px">
						...
						<table border="0" cellspacing="0" cellpadding="5" width="100%">
							<tr>
								<td><?php echo strip_tags(stripslashes($_POST["prop_abstract"])); ?></td>
							</tr>
						</table>
					</td>
				</tr>
				<tr>
					<td id="summaryTD" style="border-top: solid 1px #CCCCCC; padding: 20px">
						...
						<table border="0" cellspacing="0" cellpadding="5" width="100%">
							<tr>
								<td><?php echo strip_tags(stripslashes($_POST["prop_comments"])); ?></td>
							</tr>
						</table>
					</td>
				</tr>
