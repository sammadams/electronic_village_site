<?php
	...
	$topTitle = "Delete Presentation (Other)";
...
<?php
	...
	if(isset($_POST["del_session"])) $sesID = strip_tags($_POST["del_session"]);
	else {
	...
	$q_stmt = $db->prepare("SELECT `id`, `title`, `presenters` FROM `other_proposals` WHERE `id` = ?");
	...
	$pr_stmt = $db->prepare("SELECT `ID`,`First Name`,`Last Name` FROM `other_presenters` WHERE 1");
	$pr_stmt->execute();
	...
	if(isset($_POST["delOK"]) && $_POST["delOK"] == "Y") {
		...
		$q_stmt = $db->prepare("DELETE FROM `other_proposals` WHERE `id` = ? LIMIT 1");
		$q_stmt->bind_param('s',$propID);
		if($q_stmt->execute()) { //remove the proposal from the list in the session information
			//get the session information
			$s_stmt = $db->prepare("SELECT `presentations` FROM `sessions` WHERE `id` = ?");
			$s_stmt->bind_param('s',$sesID);
			$s_stmt->execute();
			$s_stmt->bind_result($sPres);
			$s_stmt->fetch();
			
			$tSP = explode("||",$sPres);
			$tSPStr = "";
			for($i = 0; $i < count($tSP); $i++) {
				if($tSP[$i] == $propID) continue; //skip the one we just deleted
				
				$tSPStr .= $tSP[$i]."||";
			}
			$s_stmt->close();
			
			$d_stmt = $db->prepare("UPDATE `sessions` SET `presentations` = ? WHERE `id` = ? LIMIT 1");
			$d_stmt->bind_param('ss',$tSPStr,$sesID);
			if($d_stmt->execute()) { //show the confirmation page
				include "adminTop.php";
?>
	<h3 align="center">Successfully deleted!</h3>
	<p align="center"><a href="scheduleSession.php?s=<?=$sesID?>">Back to Presentation List</a></p>
<?php
				include "adminBottom.php";
				exit();
...
?>
		...
		<script type="text/javascript">
			function cancelDelete() {
				...
				window.location.href = 'editOther.php?id=<?=$propID?>&s=<?=$sesID?>';
			}
