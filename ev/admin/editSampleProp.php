...
<?php
	...
	if(isset($_POST["prop_title"])) {
		...
		$q_stmt = $db->prepare("UPDATE `sample_proposals` SET `event` = ?, `title` = ?, `summary` = ?, `abstract` = ?, `visible` = ? WHERE `id` = ? LIMIT 1");
	
		$q_stmt->bind_param('ssssss',strip_tags($_POST['prop_event']), strip_tags($_POST['prop_title']), strip_tags($_POST['prop_summary']), strip_tags($_POST["prop_abstract"]), strip_tags($_POST["prop_visible"]), strip_tags($_POST["prop_id"]));
		...
		include "adminTop.php";
?>
	<h3 align="center">Sample proposal saved successfully!</h3>
...
<?php
	...
	$propStmt = $db->prepare("SELECT id, event, title, summary, abstract, visible FROM sample_proposals WHERE id = ?");
	$propStmt->bind_param('s', $id);
	$propStmt->execute();
	$propStmt->store_result();
	if($propStmt->num_rows < 1) {
		echo "No proposal was found with that id! (Error: ".$propStmt->error.")";
		exit();
	}
	
	$propStmt->bind_result($propID, $propEvent, $propTitle, $propSummary, $propAbstract, $propVisible);
	$propStmt->fetch();
	$propStmt->close();
...
?>
	...
	<script type="text/javascript">
		...
		function init() {
			setEvent();
			countWords(document.getElementById('proposal_summary'));
			countWords(document.getElementById('proposal_abstract'));
		}
		...
		function deleteProp() {
			window.location.href = 'deleteSampleProp.php?id=<?php echo $id; ?>';
		}
...
<?php
	foreach($events AS $e) {
?>
			<option value="<?php echo $e['id']; ?>"<?php if($propEvent == $e['id']) { ?> selected="selected"<?php } ?>><?php echo $e['event']; ?></option>
<?php
	}
?>
		</select>
	</p>
	<p style="text-align: left; border-top: solid 1px #AAAAAA; padding-top: 20px; padding-bottom: 20px;"><span style="font-weight: bold;">Title:</span> <input type="text" name="proposal_title" id="proposal_title" style="width: 100%" value="<?php echo $propTitle; ?>" /></p>
	<p style="text-align: left; border-top: solid 1px #AAAAAA; padding-top: 20px; padding-bottom: 20px"><span style="font-weight: bold;">Summary (<span id="summaryMaxWordsSpan">50</span> words maximum)</span><br /><br />
		<textarea name="proposal_summary" id="proposal_summary" rows="3" cols="100" onkeyup="countWords(this)"><?php echo $propSummary; ?></textarea><br /><br />
	...
	<p style="text-align: left; border-top: solid 1px #AAAAAA; padding-top: 20px; padding-bottom: 20px;"><span style="font-weight: bold;">Abstract (<span id="abstractMaxWordsSpan">0</span> words maximum)</span><br /><br />
		<textarea name="proposal_abstract" id="proposal_abstract" rows="5" cols="100" onkeyup="countWords(this)"><?php echo $propAbstract; ?></textarea><br /><br />
	...
	<p style="text-align: left; border-top: solid 1px #AAAAAA; padding-top: 20px; padding-bototm: 20px;">
		...
		<select id="proposal_visible" name="proposal_visible">
			<option value="Y"<?php if($propVisible == "Y") { ?> selected="selected"<?php } ?>>Yes</option>
			<option value="N"<?php if($propVisible == "N") { ?> selected="selected"<?php } ?>>No</option>
		</select>
	</p>
	<p align="center">
		<input type="button" value="Cancel" onClick="window.location.href='samplePropList.php'" /> &nbsp; &nbsp; &nbsp; &nbsp; 
		<input type="button" value="Submit" onClick="checkForm()" /> &nbsp; &nbsp; &nbsp; &nbsp; 
		<input type="button" value="Delete Proposal" onClick="deleteProp()" />
	</p>
