<?php
	...
	$prevYears = array(
		'2015' => array(
			'ts' => 'Convention Center Room 701-A: Technology Showcase',
			'ev' => 'Convention Center Room 701-B: Electronic Village',
			'oms' => mktime(16,50,0,3,26,2015),
			'ome' => mktime(23,59,59,3,26,2015),
			'pms' => mktime(15,0,0,3,27,2015),
			'pme' => mktime(23,59,59,3,27,2015)
		)
	);
	$currentYear = '2019';
	$currentEV = 'Exhibition Hall - Booth #917';
	$currentTS = 'Exhibition Hall - Booth #1111';
	
	$recordYear = (isset($_GET["y"]) && $_GET["y"] != '0') ? preg_replace("/\D/","",$_GET["y"]) : $currentYear;

	//If we are accessing a different year, then we need to connect to a different database
	if($recordYear != $currentYear) {
		$thisDB = new mysqli(HOST,USER,PASSWORD,"callisor_ev_".$recordYear);
		if(mysqli_connect_errno()) {
			echo "Failed to connect to MySQL: ".mysqli_connect_error();
			exit();
		}
		
		//get the locations from the prevYears array
		foreach($prevYears AS $y => $v) {
			if($y == $recordYear) {
				$evLocation = $v['ev'];
				$tsLocation = $v['ts'];
				break;
		...
		$thisDB = $db;
		$evLocation = $currentEV;
		$tsLocation = $currentTS;
	}
...
<?php
	...
	if(isset($_POST["sessionID"])) {
		...
		$eStmt = $thisDB->prepare("SELECT * FROM `stats` WHERE `sessionID` = ? LIMIT 1");
		$eStmt->bind_param('s',$_POST["sessionID"]);
		$eStmt->execute();
		$eStmt->store_result();
		if($eStmt->num_rows > 0) { //already exists, so update
			$uStmt = $thisDB->prepare("UPDATE `stats` SET `attendance` = ? WHERE `sessionID` = ? LIMIT 1");
			$uStmt->bind_param('ss',$_POST["attendNums"],$_POST["sessionID"]);
			$uStmt->execute();
			$uStmt->close();
		} else { //insert
			$eStmt->close();
			$iStmt = $thisDB->prepare("INSERT INTO `stats` (`sessionID`,`attendance`) VALUES (?,?)");
	...
	if(isset($_GET["s"]) && strip_tags($_GET["s"]) != "t") {
		...
		$qStmt = $thisDB->prepare("SELECT * FROM `sessions` WHERE `id` = ? LIMIT 1");
		$qStmt->bind_param('s',$sesID);
	} else {
		$qStr = "SELECT sessions.*, events.event FROM `sessions` LEFT JOIN events on events.id = sessions.event WHERE 1";
		if(isset($_GET["o"])) {
			$qStr .= " ORDER BY ";
	...
	$pStmt = $thisDB->prepare("SELECT `id`,`title`,`presenters`,`summary` FROM `proposals` WHERE 1");
	$pStmt->execute();
	...
	$prStmt = $thisDB->prepare("SELECT `ID`, `First Name`, `Last Name`, `Email`, `Affiliation Name`, `Affiliation Country`, `Publish Email` FROM `presenters` WHERE 1");
	$prStmt->execute();
	...
	$stStmt = $thisDB->prepare("SELECT * FROM `stations` WHERE 1");
	$stStmt->execute();
	...
	$aStmt = $thisDB->prepare("SELECT * FROM `stats` WHERE 1");
	$aStmt->execute();
	...
	//get the proposal information
	$opStmt = $thisDB->prepare("SELECT `id`,`title`,`presenters`,`summary` FROM `other_proposals` WHERE 1");
	$opStmt->execute();
	...
	//get the presenters information
	$oprStmt = $thisDB->prepare("SELECT `ID`, `First Name`, `Last Name`, `Email`, `Affiliation Name`, `Affiliation Country`, `Publish Email` FROM `other_presenters` WHERE 1");
	$oprStmt->execute();
	...
	//get the proposal information
	$cpStmt = $thisDB->prepare("SELECT `id`,`title`,`presenters`,`summary` FROM `classics_proposals` WHERE 1");
	$cpStmt->execute();
	...
	//get the presenters information
	$cprStmt = $thisDB->prepare("SELECT `ID`, `First Name`, `Last Name`, `Email`, `Affiliation Name`, `Affiliation Country`, `Publish Email` FROM `classics_presenters` WHERE 1");
	$cprStmt->execute();
	...
	if(isset($_GET["s"]) && strip_tags($_GET["s"]) == "t") {
		...
		foreach($prevYears AS $y => $v) {
			if($recordYear == $y) {
				$openMeetingStart = $v['oms'];
				$openMeetingEnd = $v['ome'];
				$planningMeetingStart = $v['pms'];
				$planningMeetingEnd = $v['pme'];
			}
		}
		...
		$events[$eI] = array(
			...
			"start" => $planningMeetingStart,
			"end" => $planningMeetingEnd,
			"attendance" => 0
		);
		...
		$data = file_get_contents($recordYear."_EV_TS_Attendance.csv");
		//$s = 0;
		//$e = strpos($data,"ev,");
		//$evts = false;
		//$lines = array();
...
?>
		...
		<p align="center">Showing records from <span style="font-weight: bold"><?php echo $recordYear; ?></span></p>
		<p align="center"><a href="attendance.php?y=<?php echo $recordYear; ?>">Back to Database Statistics</a></p>
...
?>
			<tr>
				<td class="<?php echo $rowClass; ?>" style="width: 150px"><?php if($events[$m]['room'] == "ts") { ?>Technology Showcase<?php } else { ?>Electronic Village<?php } ?></td>
				<td class="<?php echo $rowClass; ?>"><?php echo $events[$m]['title']; ?></td>
				<td class="<?php echo $rowClass; ?>" style="width: 100px"><?php echo date("F j",$events[$m]["start"]); ?></td>
				<td class="<?php echo $rowClass; ?>" style="width: 110px"><?php echo date("g:i",$events[$m]["start"])." - ".date("g:i A",$events[$m]["end"]); ?></td>
				<td class="<?php echo $rowClass; ?>" style="text-align: center"><?php echo $events[$m]['attendance']; ?></td>
			</tr>
<?php
	}
?>
...
<?php
		...
		/* END SCANNER DATA SECTION */
	} else if(isset($_GET["s"])) { //Data for a specific session from the database (not the scanners)
		$i = 0;
		...
		if($sessions[$i]["location"] == "ev") $locationStr = $evLocation;
		else if($sessions[$i]["location"] == "ts") $locationStr = $tsLocation;
...
?>
	...
	<h3 style="text-align: center; font-family: Arial; margin: 0"><?php echo $sessions[$i]["title"]; ?></h2>
	<h3 style="text-align: center; font-family: Arial; margin: 0"><?php echo $timeStr; ?> (<?php echo $dateStr; ?>)</h3>
	<h3 style="text-align: center; font-family: Arial; margin: 0; margin-bottom: 10px"><?php echo $locationStr; ?></h3>
...
?>
			<tr id="row_<?php echo $rN; ?>">
<?php
				if($sessions[$i]["event"] == "Technology Fairs" || $sessions[$i]["title"] == "Technology Fair Classics") {
?>
				<td id="station_<?php echo $rN; ?>" valign="top" width="80" class="<?php echo $rowClass; ?>" onClick="editNum(this)"><?php echo $pArr[$j]["station"]; ?></td>
<?php
				}
?>
				<td id="title_<?php echo $rN; ?>" valign="top" class="<?php echo $rowClass; ?>" onClick="editNum(this)"><?php echo stripslashes($pArr[$j]["title"]); ?><input type="hidden" id="propID_<?php echo $rN; ?>" value="<?php echo $pArr[$j]['id']; ?>" /></td>
				<td id="pres_<?php echo $rN; ?>" valign="top" width="200" class="<?php echo $rowClass; ?>" onClick="editNum(this)"><?php echo $presStr; ?></td>
				<td id="num_<?php echo $rN; ?>" valign="top" width="110" class="<?php echo $rowClass; ?>" style="text-align: center" onClick="editNum(this)"><?php echo $pArr[$j]["attendance"]; ?></td>
			</tr>
<?php
				$rN++;
			}
...
?>
	...
	<div id="footer">
		<p id="saveMsg" align="center"><?php if(isset($_POST['sessionID'])) { ?><span style="color: green">CHANGES SAVED!</span> <input type="button" value="Back to Session List" onClick="window.location.href='attendance.php'" /><?php } else { ?>&nbsp;<?php } ?></p>
	</div>
	<form name="attendanceForm" id="attendanceForm" method="post" action="attendance.php?y=<?php echo $recordYear; ?>">
		<input type="hidden" name="sessionID" id="sessionID" value="<?php echo $sessions[$i]['id']; ?>" />
...
<?php
		...
		for($j = 0; $j < count($pArr); $j++) {
			if(count($pArr[$j]) > 0) {
?>
		attend[<?php echo $rN; ?>] = <?php echo $pArr[$j]['attendance']; ?>;
<?php
				$rN++;
			}
		}
...
?>
	...
	<script type="text/javascript">
		...
		function doPrint(n) {
			var url = 'attendance.php?s=' + n + '&y=<?php echo $recordYear; ?>';
...
<?php
	...
	if($recordYear != $currentYear) { //there is scanner data to show
?>
	<p align="center"><a href="attendance.php?s=t&y=<?php echo $recordYear; ?>">See statistics from badge scanners at doors</a></p>
<?php
	}
?>
...
?>
		<tr id="session<?php echo $e; ?>_row<?php echo $rN; ?>">
			<td class="<?php echo $rowClass; ?>" width="300" onMouseOver="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',0)" onClick="doPrint('<?php echo $sessions[$i]['id']; ?>')"><?php echo $sessions[$i]['title']; ?></td>
			<td class="<?php echo $rowClass; ?>" width="150" onMouseOver="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',0)" onClick="doPrint('<?php echo $sessions[$i]['id']; ?>')"><?php echo $dateStr; ?></td>
			<td class="<?php echo $rowClass; ?>" width="150" onMouseOver="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',0)" onClick="doPrint('<?php echo $sessions[$i]['id']; ?>')"><?php echo $timeStr; ?></td>
			<td class="<?php echo $rowClass; ?>" style="text-align: center" width="100" onMouseOver="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',0)" onClick="doPrint('<?php echo $sessions[$i]['id']; ?>')"><?php echo $pCount; ?></td>
			<td class="<?php echo $rowClass; ?>" style="text-align: center" width="100" onMouseOver="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $e; ?>','<?php echo $rN; ?>',0)" onClick="doPrint('<?php echo $sessions[$i]['id']; ?>')"><?php echo $sessions[$i]['total_attendance']; ?></td>
		</tr>
<?php
				$rN++;
			}
...
?>
	<table border="0" align="center" cellpadding="5" width="800">
		<tr>
			<td colspan="3"><?php echo $eTypes[$e]; ?> (Total #: <?php echo $rN; ?>)</td>
		</tr>
		<tr>
...
?>
	...
	<script type="text/javascript">
		window.addEventListener('load',function() {
			var tsTotal = <?php echo $tsTotal; ?>;
			var evTotal = <?php echo $evTotal; ?>;
...
<?php
	...
	function printPrevYears() {
		global $recordYear, $prevYears, $currentYear;
?>
	<script type="text/javascript">
		function getRecords(el) {
			window.location.href = 'attendance.php?y=' + el.options[el.selectedIndex].value;
		}
	</script>
	<p style="text-align: center">
		Showing records from <span style="font-weight: bold"><?php echo $recordYear; ?></span>
		(See records from 
		<select id="recYear" name="recYear" onchange="getRecords(this)" >
