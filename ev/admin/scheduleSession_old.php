...
<?php
	...
	$evtStmt = $db->prepare("SELECT id, event, adminSuffix FROM events WHERE isActive = 'Y'");
	$evtStmt->execute();
	$evtStmt->bind_result($evtID, $evtEvent, $evtAdminSuffix);
	...
	if(isset($_GET["s"])) { //a specific session id was given
		...
		$sStmt = $db->prepare("SELECT s.id, l.name AS location, s.date, s.time, e.event AS event, s.title, s.presentations FROM sessions AS s LEFT JOIN events AS e ON e.id = s.event LEFT JOIN locations AS l ON l.id = s.location WHERE s.id = ?");
		echo $db->error;
		...
		if($thisSession["event"] != "Other") {
			$pStmt = $db->prepare("SELECT `id`,`title`,`contact`,`presenters`,`times`,`topics`,`computer`,`summary`,`abstract`,`password`,`salt`,`comments`,`photoOK`,`emailOK`,`type`,`status`,`confirmed` FROM `proposals` WHERE 1");
			$pStmt->execute();
			$pStmt->bind_result($pID, $pTitle, $pContact, $pPres, $pTimes, $pTopics, $pComputer, $pSummmary, $pAbstract, $pPass, $pSalt, $pComments, $pPhotoOK, $pEmailOK, $pType, $pStatus, $pConfirmed);
			$proposals = array();
			while($pStmt->fetch()) {
				if($pType == $thisSession["event"] && $pStatus == "accepted") {
					$proposals[] = array(
						"id" => $pID,
						"title" => $pTitle,
						"presenters" => $pPres,
			...
			$prStmt = $db->prepare("SELECT `ID`,`First Name`,`Last Name` FROM `presenters` WHERE 1");
			$prStmt->execute();
			...
			$sesStmt = $db->prepare("SELECT `id`,`location`,`date`,`time`,`event`,`title`,`presentations` FROM `sessions` WHERE 1");
			$sesStmt->execute();
			$sesStmt->bind_result($sesID,$sesLocation,$sesDate,$sesTime,$sesEvent,$sesTitle,$sesPres);
			$sessions = array();
			while($sesStmt->fetch()) {
				if($sesEvent == $thisSession["event"]) {
					$sessions[] = array(
						"id" => $sesID,
		...
		} else { //get the Other events information
			if(strpos($thisSession["title"],"Technology Fair: Classics") !== false) {
				/*
					The Classics are like the Technology Fairs, but are not soliticted. So we need to deal with them in
					scheduling like we do the Technology Fairs.
				 */
				...
				/*
					"Other" event presentations are only tied to a specific event in the schedule and proposals were
					not submitted for any of these events. The information for each event is provided by the organizers
					as needed. For each event, we need the following information for the Program Book:
				
						- Title
						- Presenters' information (name, email, etc.)
						- Summary
				
					When the user clicks on an "Other" session, they will see the list of presentations tied to this event.
					They will also see a link to add a presentation, which will bring up a page where they can enter the
					information listed above.
				 */
			
				//First, get all presentations tied this particular session
				$oStmt = $db->prepare("SELECT `id`,`presenters`,`title`,`summary` FROM `other_proposals` WHERE 1");
				$oStmt->execute();
				...
				$opStmt = $db->prepare("SELECT `ID`,`First Name`,`Last Name` FROM `other_presenters` WHERE 1");
				$opStmt->execute();
				$opStmt->bind_result($opID,$opFirstName,$opLastName);
				$presenters = array();
				while($opStmt->fetch()) {
					$presenters[] = array(
						"id" => $opID,
...
?>
		<tr id="row<?php echo $rN; ?>">
			<td class="<?php echo $rowClass; ?>" width="50" valign="center" align="center" onMouseOver="highlightRow('<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $rN; ?>',0)"<?php if($otherSession != 0) { ?> onClick="editSession('<?php echo $otherSession; ?>')"<?php } else { ?> onClick="checkBox('<?php echo $rN; ?>')"<?php } ?>><?php echo $assignedStr; ?></td>
			<td class="<?php echo $rowClass; ?>" width="450" onMouseOver="highlightRow('<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $rN; ?>',0)" onClick="checkBox('<?php echo $rN; ?>')"<?php if($otherSession != 0) { ?> onClick="editSession('<?php echo $otherSession; ?>')"<?php } else { ?> onClick="checkBox('<?php echo $rN; ?>')"<?php } ?>><?php echo $proposals[$i]['title']; ?></td>
			<td class="<?php echo $rowClass; ?>" width="250" onMouseOver="highlightRow('<?php echo $rN; ?>',1)" onMouseOut="highlightRow('<?php echo $rN; ?>',0)" onClick="checkBox('<?php echo $rN; ?>')"<?php if($otherSession != 0) { ?> onClick="editSession('<?php echo $otherSession; ?>')"<?php } else { ?> onClick="checkBox('<?php echo $rN; ?>')"<?php } ?>><?php echo $presStr; ?></td>
		</tr>
<?php
...
<?php	
		...
		} else if($thisSession["event"] == "Other") {
			if($thisSession["title"] == "Technology Fair: Classics") {
?>
	<style type="text/css">
		th.pList {
			background-color: #333333;
